...
public class Main {
    ...
    enum RomArab {
        Arab(1), Rom(2), NoElse(0), Err(-1); //флаг сост. и типа вычислений: арабскими/ римскими /ни то ни се(ошибка)
        private int val;
            ...
            while (sc.hasNext()) {
                ...
                while (matcher.find()) {
                    ...
                    if (matcher.start(1) >= 0) {  //определитель арабских
                        ...
                        if (onlyFrom0to10 && (ival < 0 || ival > 10))
                            throw new NoValidateTipExeption("Только от 0 до 10");
                    ...
                    if (matcher.start(2) >= 0) { //определитель римских цифр
                        ...
                        ival = StringParser.toInt(si);
                        if (onlyFrom0to10 && (ival < 0 || ival > 10))
                            throw new NoValidateTipExeption("Только от 0 до 10ти!");
                        if (only2digit && (cntDig > 2)) throw new NoValidateTipExeption("Только 2 числа!");
